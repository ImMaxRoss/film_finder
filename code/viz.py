...
def word_plot(
    ...
    for i, sentiment_df in enumerate(data_by_sentiment):
        ...
        all_words_in_tweet = sentiment_df[value_column].apply(lambda x: x.split(' ')).explode()
    ...
    all_words_ = data[value_column].apply(lambda x: x.split(' ')).explode()
    ...
    ax[-1].set(
        title="Top Words In Description"
    )
    plt.xticks(rotation=15)
    plt.suptitle("Description-Word Breakdown by Genre")
